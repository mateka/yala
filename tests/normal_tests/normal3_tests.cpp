...
TEMPLATE_TEST_CASE("normal3 can be accessed with custom accessors", "[vec, vec3, normal, normal3]",
    ...
    SECTION("Cross product can be calculated") {
        constexpr normal i = {TestType(1), TestType(0), TestType(0)};
        constexpr normal j = {TestType(0), TestType(1), TestType(0)};
        constexpr normal k = {TestType(0), TestType(0), TestType(1)};
